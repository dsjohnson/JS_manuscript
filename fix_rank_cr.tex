...
To begin moving towards performing this approach in {\tt nimble} we need to start forming distributions for a model description. So, we denote the distribution function in (\ref{eq:chmm}) as the conditional HMM, $[\bx_i|\bt,n]=\text{cHMM}(\bx_i|\bt)$. Then the hierarchical model components that need to be specified are
\[
