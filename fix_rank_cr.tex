...
Here we present an example analysis with a multistate component, abundance of female nesting Hawaiian green sea turtles ({\it Chelonia mydas}) or {\it honu}. This analysis demonstrates that we can still use the fully marginal collapsed sampler to fit models that incorporate individual by time effects, specifically we fitted a model with an effect of previous captures on the capture probability of an individual. In addition, these data are relatively large and PX-DA methods can be computationally expensive. 

The data presented here are from the 1973--2016 nesting season surveys on the islets within the atoll of Lalo (French Frigate Shoals), Northwestern Hawaiian Islands. Over the 44 year study period $n$ = 5,271 females were captured. During the season, honu will migrate from the Main Hawaiian Islands (MHI) to nest at Lalo starting in late April through early August. After nesting is completed honu will return to the MHI and enter the foraging phase which lasts between 2--10 (average {\it c.} 4) years, where no nesting occurs \citep{BalazsEtAl2015a}. The time between nesting occasions is termed the {\it remigration interval}. 

There are two alterations to the traditional JS model that are necessary to estimate the size of the nesting female population with this data. The main issue is that individuals become unavailable for capture for an extended time while they are foraging as it is not feasible to capture individuals throughout the entire MHI. During nesting phase individuals are concentrated at Lalo, thus, are relatively easy to capture. Secondly, processing individuals on their initial capture (unmarked) is more time consuming than recording individuals that already have a permanent identification (marked). In some years there is not enough total survey resources to equally processed marked and unmarked individuals. Therefore, in some years capture and processing of unmarked individuals is a lower priority than recording individuals that were previously marked. This is similar to the closed-population $M_b$ model where the individuals become ``trap happy'', however, this mathematical behavioral response is due to capture effort on the part of the researchers.

First we consider the reproductive structure. To model the effect of the remigration interval on abundance of hawksbill marine turtles ({\it Eretmochelys imbricata}) \citet{kendall2019multistate} used a version of the breeding return time (JS-BRT) model of \cite{PledgerEtAl2013a}. The JS-BRT model uses several intermediate (sub-) states to represent the overall condition of the individual being in a nonbreeding state, e.g., for honu, the individual is in a foraging (nonbreeding) state if it is in any one of the substates, $F_1,\dots, \text{or } F_m$. The transition probabilities force the individual to pass through the substates sequentially or return to the reproductive (nesting) state. This substate trick is used to build a Hidden {\it semi}-Markov model (HSMM) rather than HMM, because, in this case, the dwell-time in the overall foraging state is not geometrically distributed as it would be in a Markov chain. Here we used $m=7$ substates to approximate a Poisson+1($\mu$) distribution for dwell time in the foraging state \citep{langrock2011hidden}. The Poisson+1($\mu$) distribution is defined at a Poisson variable with rate $\mu$ where 1 is added, so, the expected value is $\mu+1$ with variance $\mu$.

Now we examine the pseudo trap response issue. In a PX-DA model this would be taken care of with a individual $\times$ occasion covariate, say $c_{i,t}$ that indicates if individual $i$ has been previously captured at time $t$. We cannot use that approach in the collapsed sampler because we are not explicitly simulating capture-histories for uncaptured individuals. However, we can use the another set of substates to define our reproductive state: $R_{00}$, $R_{01}$, $R_{10}$, $R_{11}$, where the subscripts indicate, respectively, previously captured and currently captured on occasion $t$. Thus, we have moved the capture process into the state transitions. In addition to the expanded reproductive state, we also need to further expand the foraging states to $F_{01},\dots,F_{0m},F_{11},\dots,F_{1m}$ where the first subscript denotes previous capture. These additional foraging states are not biologically different, they simply serve to prevent transitions among the reproductive states that are not permitted, e.g., if an individual is in $R_{00}$ its next reproductive state cannot be $R_{11}$ or $R_{10}$.

The model specifications for this model are notably different from the standard JS form described previously. However, It can still be placed in the same HMM framework for which we can use the collapsed Gibbs sampler with the additional computational savings benefits. Here we provide an overview of the matrix specifications, more details are found in Appendix B. 

We begin with the transition probability matrix can be represented with the block structure,
\[
\bG_t = \left[\begin{array}{cccc}
1-\tilde{\xi}_{t+1} & \tilde{\xi}_{t+1}\boldsymbol{\Psi}_{\emptyset,R,t+1} & \mathbf{0} & 0\\
\mathbf{0} & \mathbf{0} & \phi\boldsymbol{\Psi}_{R,F,t}  & (1-\phi)\mathbf{1} \\
\mathbf{0} & \phi\boldsymbol{\Psi}_{F,R,t} &  \phi\boldsymbol{\Psi}_{F,F,t} & (1-\phi)\mathbf{1} \\
0 & 0 & 0 & 1
\end{array}
\right],
\] 
where the $\boldsymbol{\Psi}_{a,b,t}$ matrices represent the submatrix describing transitions from the composite state ``$a$'' to composite state ``$b$''. This transition matrix is similar to the HSMM capture recapture model of \cite{king2016semi} with the exception that we added the entry process as well. The composite states are $R$ with four substates and $F$ with 14 substates which makes each $\bG_t$ a $20 \times 20$ dimensional matrix. The entry submatrix is, $1\times 4$ where $\boldsymbol{\Psi}_{\emptyset,R,t} = [1-\alpha_t p_t\ \ \alpha_t p_t\ \ 0\ \ 0]$, where $p_t$ is the capture-probability and $0<\alpha_t < 1$ is the proportional reduction in capture probability due to reduced effort adding new individuals. The submatrix $\boldsymbol{\Psi}_{R,F,t}$ is a $4 \times 14$ indicator matrix such that $R_{00}$ transitions to $F_{01}$ and $R_{01}$, $R_{10}$, and $R_{11}$ transition to $F_{11}$ automatically. The submatrices $\boldsymbol{\Psi}_{F,F,t}$ and $\boldsymbol{\Psi}_{F,R,t}$ work together to allow transitions to proceed through foraging an and exit to a and appropriate reproductive state such that the number of occasions spent foraging is approximately Poisson+1 distributed \citep{langrock2011hidden}. The $\boldsymbol{\Psi}_{F,R,t}$, matrix is composed of elements,
\[
...
\]
for $j=1,\dots,m$ where $\tau_j$ is the probability of transitioning from foraging substate $j$ to a reproductive state (see Appendix B). All other $F$ to $R$ transitions have probability zero. The $\boldsymbol{\Psi}_{F,F,t}$ matrix is composed of elements,
\[
\psi_{F_{0j},F_{0,j+1},t} = 1-\tau_j \text{ for } j=1,\dots,m-1, \text{ and } \psi_{F_{0m},F_{0m},t} = 1-\tau_{k}.
\]
The $F_1$ transitions are identical and all other probabilities are 0.

The other HMM matrices are 
\[
\bpi = \left[1-\tilde{\xi}_1\quad \tilde{\xi}_1\boldsymbol{\Psi}_{\emptyset,R,1}\quad \mathbf{0}\quad  0\right]
\]
and $\bP(x_{i,t})$ is still a diagonal matrix with with $[x_{i,t}|s_l]$ entries. However, because the detection process has been absorbed in the states, $[x_{i,t}|s_l]$, is just an indicator that the observation $x_{i,t}$ is possible in state $s_l$. For example $[x_{i,t}=0|s_l=R_{00}]=1$ and  $[x_{i,t}=0|s_l=R_{01}]=0$. 

Now that we the general form of the HMM matrices, we can define specific models for the JS parameters. For this analysis we modeled survival probability using a logit trend,
\[
\text{logit}(\phi_t) = \beta_{\phi0} + \beta_{\phi}\cdot t,
\]
with prior distributions, $[\beta_{\phi0}] = N(0,1.5^2)$ and $[\beta_{\phi}] = N(0,\sigma_{\phi}^2)$, where $[\sigma_{\phi}]=\text{Exponential}(1)$. The entrance probabilities are modeled using the recruitment probabilities of \cite{xxx}. The recruitment probability, $\rho_t$ is the probability that the individual enters at occasion $t$ given it has not yet entered. This is the essentially the reverse of the exit probability. To use it in the collapsed sampler we need,
\[
\xi_t \propto  \rho_t \prod_{j=1}^{t-1} (1-\rho_j).
\]
This gives us the unconditional probability of entry from which we obtain $\tilde{\xi}_t$ for the transition matrices. Recruitment was modeled with a Gaussian radial basis function.
\[
\text{logit}(\rho_t) = \beta_{\rho0} + \mathbf{b}_t\bb_{\rho}
\] 
where $[\beta_{\rho0}] = N(0,1.5^2)$, $\mathbf{b}_t$ is a 20 knot Gaussian radial basis function evaluated at $t=1,\dots,K$, $[\beta_{\rho,j}] = N(0,\sigma_{\rho}^2)$ for $j=1,\dots,20$, and $[\sigma_{\rho}]=\text{Exponential}(1)$. The capture probabilities for previously captured individuals is modeled as,
\[
