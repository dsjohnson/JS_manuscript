...
One of the major distinguishing features of JS family models relative to other capture-recapture models, such as Cormack-Jolly-Seber (CJS) models, is the entry probability. \citet{schwarz1996general} and \cite{crosbie1985parsimonious} parameterized entry in the JS model using marginal multinomial probabilities, $\xi_t$, $t=1,\dots,K$,  which are the unconditional probabilities that an individual enters between occasions $t-1$ and $t$ and is available for capture on occasion $t$. There are other ways to think about entry probability but for now, we will base our development on the traditional multinomial version. See Section 4.2 for an alternate example using the dynamic occupancy parameterization of  \citealt{Royle:2008kx}. With the $\xi_t$ entry probabilities we can form the {\it conditional} entry probabilities that will used in the JS model computations herein,
\[
...
To compare the relative run-times of each of the methods we hold PX-DA (300) as the gold-standard. The extra 700 augmented individuals in the second PX-DA increased run-time by 3.68$\times$. Run-time for the standard collapsed sampler was 1.82$\times$. Finally, the run-time of the collapsed sampler with computational savings was 0.11$\times$, a substantial improvement over the PX-DA methods. For the dipper data there are only $u=32$ unique capture-histories in the $n=294$ observed individuals. So, the likelihood calculation only needs to loop over the 32 unique histories for each MCMC iteration. 

Parameter estimates  are given in Table \ref{tab:dipper} for the standard collapsed sampler. For each of the four methods, the parameter and abundance estimates aligned outside of Monte Carlo error as expected because it is the same model fitted. The super population size estimate is $N = 324.6$ with SD = 16.3 and 95\% highest probability density interval of [300--356]. Yearly abundance estimates are illustrated in Figure \ref{fig:dipper.Nt}. 
%
