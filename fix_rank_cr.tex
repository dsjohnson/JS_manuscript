...
\] 
which is the probability that an individual enters between $t-1$ and $t$ given it has not entered prior to $t-1$ but will enter before $K$. Note that we only need to model the $\xi_t$ up to a normalizing constant to calculate $\tilde{\xi}_t$. Also, note that the $\sum_j \xi_j = 1$ constraint is reflected in the fact that $\tilde{\xi}_K \equiv 1$. Given a set of $\tilde{\xi}$ conditional probabilities we can obtain the unconditional $\xi$ parameters with the relationship,
\[
...
\subsection{Additional computation benefits} \label{sec.savings}

Here we demonstrate some additional computational savings that we can use due to the marginalization and factoring of the posterior distribution in the collapsed Gibbs sampler. Using the previous development, we can write the full JS model posterior distribution as,
\begin{equation}
...
\begin{equation}
[\bx|\bt,n] = \prod_{i=1}^n [\bx_i|\bt,n] = \prod_{j=1}^m w_j[\bu_i|\bt,n]
\end{equation}
where $\bu_j$ is the $j$th unique capture-history and $w_j$ is the number of times it occurs in $\bx$. This is not new and is standard practice when fitting models using maximum likelihood methods, however, it is not possible with the PX-DA approach for Bayesian analysis. This can be a substantial computational savings if $m<<n$. 

The second computational saving is due to the predictive factoring in (\ref{eq.post.factor}). This is the same factoring approach used by \cite{Hooten:2023aa}, and \cite{hooten2024geostatistical} to facilitate an efficient 2-stage approach to Bayesian estimation of closed-population models and we can do the same here. The 2-stage approach first involves drawing a posterior sample from $[\bt,\lambda|\bx, n]$, preferably using just the unique capture-histories. Then, divide the sample into $B$ batches and sample from $[\bvt,n_u|\bt,\lambda,\bx]$ in parallel for each $(\bt,\lambda)$ in the separate batches. Of course, if occasion-specific abundance estimates ($\bvt$) are not desired, one would just sample $[n_u|\bt,\lambda,\bx] = \text{Poisson}(\lambda(1-p^*))$ in the first stage and stop there. 
...
To compare the relative run-times of each of the methods we hold PX-DA (300) as the gold-standard. The extra 700 augmented individuals in the second PX-DA increased run-time by 3.68$\times$. Run-time for the standard collapsed sampler was 1.82$\times$. Finally, the run-time of the collapsed sampler with computational savings was 0.11$\times$, a substantial improvement over the PX-DA methods. It is even more impressive comparing it to the standard collapsed sampler (0.06$\times$). For the dipper data there are only ($m=32$) unique capture-histories in the $n=294$ observed individuals. So, the likelihood calculation only needs to loop over the 32 unique histories for each MCMC iteration. 

Parameter estimates  are given in Table \ref{tab:dipper} for the standard collapsed sampler. For each of the four methods, the parameter and abundance estimates aligned outside of Monte Carlo error as expected because it is the same model fitted. The super population size estimate is $N = 325$ with SD = 16.25 and 95\% highest probability density interval of [300--356]. Yearly abundance estimates are illustrated in Figure \ref{fig:dipper.Nt}. 
%
...
There are two alterations to the traditional JS model that are necessary to estimate the size of the nesting female population with this data. The main issue is that individuals become unavailable for capture for an extended time while they are foraging as it is not feasible to capture individuals throughout the entire MHI. During nesting phase individuals are concentrated at Lalo, thus, are relatively easy to capture. Secondly, processing individuals on their initial capture (unmarked) is more time consuming than recording individuals that already have a permanent identification (marked). In some years there is not enough total survey resources to equally processed marked and unmarked individuals. Therefore, in some years capture and processing of unmarked individuals is a lower priority than recording individuals that were previously marked. This is similar to the closed-population $M_b$ model where the individuals become ``trap happy'', however, the behavioral response is due to the people conducting the captures.

First we consider the reproductive structure. To model the effect of the remigration interval on abundance of hawksbill marine turtles ({\it Eretmochelys imbricata}) \citet{kendall2019multistate} used a version of the breeding return time (JS-BRT) model of \cite{PledgerEtAl2013a}. The JS-BRT model uses several intermediate (sub-) states to represent the overall state of the individual being in a nonbreeding state, e.g., for Lalo honu, the individual is in a nonbreeding (foraging) state if it is in any one of the substates, $S_t = F_1,\dots, \text{or } F_m$. The transition probabilities force the individual to pass through the substates sequentially or return to the reproductive (nesting) state. This substate trick is used to build a Hidden {\it semi}-Markov model (HSMM) rather than HMM, because, in this case, the dwell-time in the foraging state is not geometrically distributed \citep{langrock2011hidden,king2016semi}. 

Now we examine the trap response issue. In a PX-DA model this would be taken care of with a individual $\times$ occasion covariate, say $c_{i,t}$ that indicates if individual $i$ has been previously captured at time $t$. We cannot use that approach in the collapsed sampler because we are not explicitly simulating capture-histories for uncaptured individuals. However, we can use the another set of substates to define our reproductive state: $R_{00}$, $R_{01}$, $R_{10}$, $R_{11}$, where the subscripts indicate, respectively, previously captured and currently captured on occasion $t$. Thus, we have moved the capture process into the state transitions. 


The transition probability matrix can be represented with the block structure,
\[
...
\right],
\] 
where the $\boldsymbol{\Psi}_{a,b,t}$ matrices represent the submatrix describing transitions from the composite state ``$a$'' to composite state ``$b$''. See Appendix B for details on matrix dimensions and content for the honu analysis. 





1.313867 hours for first part second was 9.413192 mins
