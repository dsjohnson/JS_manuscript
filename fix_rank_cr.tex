...
{\Large Bayesian Estimation for Open Capture-Recapture Models Without Data Augmentation} 
...
The overall goal of this paper is to demonstrate the collapsed approach using the MCMC software in the {\tt R} package {\tt nimble} \citep{Valpine:2017vg} to illustrate that PX-DA methods are not necessary and JS models can be fit without data augmentation. The {\tt nimble} package allows users to create their own functions which can be compiled and included within an MCMC. Using this capability, as well as, the HMM likelihoods within the {\tt R} package {\tt nimbleEcology} \citep{GoldsteinEtAl2024a}, we analyze 3 real data sets with JS models of varying complexity as examples of practical JS fitting that can readily be used by practitioners just as easily as PX-DA methods. 
...
Collapsed Gibbs samplers offer a method to improve MCMC convergence over standard Gibbs samplers. \cite{Liu1994a} provides a mathematical proof which shows that the rate of convergence for a collapsed Gibbs sampler is at least as good, but often better, than the standard Gibbs sampler for any particular model.  In general a collapsed samplers is constructed by marginalizing over some components of the model, using a standard Gibbs sampler to sample the remaining parameters, then the marginalized parameters are sampled using their posterior predictive distribution given the first sample. 

For the time being, we will present a very heuristic description of the collapsed Gibbs sampler for augmented models. Details for JS models will be added in subsequent sections. Let $\bx$ represent observed data, $\bz$ is augmented data that is not directly observed, and $\bt$ are the parameters of the model. The standard Gibbs sampler for the posterior distribution,
\[
[\bt,\bz|\bx] \propto [\bx,\bz|\bt][\bt],
\]
proceeds by alternate sampling between the sequential conditional distributions: $[\bt|\bz, \bx]$ and $[\bz|\bt,\bx]$. Throughout the paper, we use the ``$[A|B]$'' notation to represent a conditional density (distribution) function of $A$ given $B$. The collapsed Gibbs sampler begins by drawing from the marginal posterior distribution,
\[
[\bt|\bx] \propto  [\bx|\bt][\bt].
\]
For JS models, this would be a standard Bayesian analysis using the JS likelihood. Then, drawing from the posterior predictive distribution $[\bz|\bt,\bx]$ provides us with a sample from the same joint posterior,
\[
[\bt,\bz|\bx] \propto [\bz|\bt,\bx][\bt|\bx],
\]
This factorization was used by \cite{Hooten:2023aa} and \cite{hooten2024geostatistical} to develop sequential Bayesian inference for closed capture-recapture models. Now we can define specifics and predictive sampling methods for JS family models. 

\subsection{Notation for a Jolly-Seber family models}

In a JS model the population is described as $N$ individuals that are available for capture in at least one capture occasion during the study period. The occasion specific abundance, $N_t$, is the size of the population available for capture on occasion $t=1,\dots,K$. Traditionally, data for the JS model are composed of capture histories for each individual observed at least once, $\bx_i = (x_{i1},\dots,x_{iK})$ where $x_{it} \in \{0,1\}$ depending on whether or not individual $i=1,\dots,n$ was captured on occasion $t$. However, we expand this notation in a general way to cover other models such as robust design and multistate JS models. For example, $x_{it}$ might represent the number of times the animal was captured in $R_t$ sub-occasions during primary occasion $t$ which defines a robust design model. Or, $x_{it}$ might be recorded as the individual being in a particular state if it is captured, implying a multistate model \citep{xxx}. Technically all JS models are multistate with states, ``pre-entry" (pre-birth), ``available", and ``post-exit" (post-death). For this paper we expand the set of states so that we may include general multistate models as well. Therefore, an individual can be in one of $J$ states, $s_1,\dots,s_J$ where $s_1$ is the pre-entry state and $s_J$ is the post-exit state. The notation $z_{it} \in \{s_1,\dots,s_J\}$ will represent the true state of individual $i$ on occasion $t$, which may, or may not, be observed accurately upon capture. 
...
\]
We prefer the recruitment probability parameterization of the JS model because the constant model, i.e., $\rho_t=\rho$, has biological interpretation about recruitment over time, however the constant model for $\beta_t$ implies changing recruitment probabilities that are not readily apparent. An alternative $\beta_t$ model is presented by \cite{link2005modeling} based on {\it per capita} recruitment parameters. We will not consider that model here, but it can just as easily be incorporated in the collapsed sampler. 

Now we proceed in describing how an individual enters into the population and transitions between states and exits. When the individual first enters the available population between occasions $t-1$ and $t$ it enters into state $s_l$, $l=2,\dots,J-1$ with probability $\alpha_{t,l}$ ($\sum_{l=2}^{J-1} \alpha_{t,l}=1$). The probability that an individual does not exit the population between occasions $t$ and $t+1$ given it is currently available for capture in state $s_l$ on occasion $t$ is $\phi_{t,l}$, $l=2,\dots,J-1$ and $t=1,\dots,K-1$. Finally, $\psi_{t,l,l'}$ ($\sum_{l'=2}^{J-1}\psi_{t,l,l'} = 1$) is the probability that an individual transitions from state $s_l$ to $s_{l'}$ between times $t$ and $t+1$ given it was in the available population at time $t$ . The exit probability $\phi_{t,l}$ can be thought of as a transition probability from state $s_l$ to $s_J$, the absorbing exit state just as $\xi_t$ is the transition probability out of $s_1$. 



\subsection{HMM formulations of Jolly-Seber Family Models}

\cite{glennie2019open} and \cite{mcclintock2020uncovering} show an individual capture history in a JS models can be formulated as a Hidden Markov Model (HMM). That is, the probability of an individual capture-history can be calculated with the matrix product,
%
\begin{equation}
\label{eq.uncond.lik}
[\bx_i|\bvt] = \bpi\bP_1(x_{i1})\bG_1\bP_2(x_{i2}) \cdots \bG_{K-1}\bP_k(x_{iK})\mathbf{1}.
\end{equation}
where $\bvt$ represents the collection of parameters in the model, $\bpi$ is the initial state probability vector, $\bP_t(x)$ is a diagonal matrix of observation likelihoods for each state, $\bG_t$ is the probability transition matrix for states, and $\mathbf{1}$ is a vector of ones. 
...
%\]
%where $r_t$ is the Poisson process detection rate. 
...
Now that we have a probabilistic model for generating capture-recapture data we can formulate the likelihood for a set of observed data. First recall that only the individuals that are captured at least once (i.e., $\bx_i \ne \mathbf{0}$) are represented in the capture histories. Therefore, to model the full process that produces the observed data the probability of capture has to be included. Thus the likelihood for $n$ observed individuals is,
\begin{equation}
\label{eq.lik}
\begin{aligned}
[\bx|\bvt,N] &= \prod_{i=1}^n [\bx_i|\bvt, \bx_i\ne\mathbf{0}][\bx_i\ne\mathbf{0}|\bvt, N] \\
&\propto \text{Binomial}(n|N,p_*) \times \prod_{i=1}^n \frac{[\bx_i|\bvt]}{p_*},
\end{aligned}
\end{equation}
where $p_* = 1- [\bx=\mathbf{0}|\bvt]$ is the probability of being captured at least once. \cite{king2015capture} note that the interpretation that the likelihood is the product of the likelihood of capturing $n$ of $N$ individuals at least once multiplied by the likelihood of capture histories conditioned on at least one capture. Readers should note that we have left the portion of the original JS models dealing with loss upon capture. We find this part is often omitted in practice. If needed, it can be included in the model using a either second absorbing state in the transition matrices or a nontrivial observation likelihood, $[x_{it}|s_J]$, to allow, respectively, (1) death upon capture to be modeled as a separate process from natural mortality or (2) acknowledge that the exit (death) state can be explicitly observed with some probability. 
 

\subsection{A collapsed Gibbs Sampler for Jolly-Seber models}

The default general form of the collapsed Gibbs sampler for JS models is:
\begin{enumerate}
\item Sample from the marginal posterior of the parameters, $[\bvt, N|\bx] \propto [\bx|\bvt,N] [\bvt] [N]$, where $[\bvt]$, $[N]$ are the parameter prior distributions.
\item Sample from the posterior predictive distribution state summaries, $[f(\bz) |\bvt, N, \bx]$ \
\end{enumerate}
Note, that because the dimension of the state matrix, $\bz$, changes with $N$ the actual state matrix is not meaningful for inference on its own, fixed dimensional summaries such as, say, abundance of individuals in a particular state are the meaningful inferential targets. 

Let us begin with step 1 and make a modification that will allow us to move sampling of $N$ to the second predictive stage as well. Under the PX-DA model, the marginal prior for the superpopulation abundance is $[N] = \text{discrete Uniform}(0,M)$ where $M$ is the number of augmented individuals. However, we do not need to add any individuals, so choosing a prior with a fixed upper bound would defeat the purpose of this exercise. Therefore we follow the advice of \cite{link2013cautionary} and \cite{schofield2023estimating} and use the objective objective scale prior $[N] = N^{-1}$ which can be approximated within an MCMC using the hierarchical specification,
\[
[N|\lambda][\lambda] = \text{Poisson}(N|\lambda)\times  \text{Gamma}(\lambda|\epsilon, \epsilon),
\]
where $\epsilon$ is a chosen value near zero. Because we assume a conditional Poisson prior distribution for $N$ one can easily marginalize over it as well to further collapse the sampler,
\begin{equation}
\begin{aligned}
[\bvt, \lambda|\bx] &\propto \sum_N[\bx|\bvt, N] \times \text{Poisson}(N|\lambda) \times [\bvt] [\lambda]\\
&\propto \text{Poisson}(n|\lambda p_*) \prod_{i=1}^n \frac{[\bx_i|\bvt]}{p_*}\ [\bvt] [\lambda]
\end{aligned}
\end{equation}
This is equivalent in form to the likelihood derived by \cite{glennie2019open} for spatial capture-recapture models when individuals are distributed as a spatial Poisson process. It might seem rather trivial to exchange a Binomial distribution for a Poisson one, however, after this marginalization, the number of undetected animals in the population, $n_u=N-n$, has the straightforward posterior predictive distribution,
\[
[n_u|\bvt, \lambda] = \text{Poisson}(\lambda(1-p_*)),
\]
conditionally independent of the data, $\bx$. So, for each iteration of an MCMC algorithm it can be directly updated with a draw from a standard Poisson distribution as a posterior predictive variable. Then the prediction for the overall abundance, $N$, results directly from $N = n_u + n$. \cite{schofield2023estimating} and \cite{johnson2010model} both describe abundance estimation in the Poisson approach as {\it predicting} the number of undetected individuals. Therefore, the collapsed sampler now has the form:
\begin{enumerate}
\item Sample from $[\bvt, \lambda|\bx]$
\item Sample from $[n_u|\bvt, \lambda]$, calculate $N = n_u + n$
\item If needed sample from  $[f(\bz) |\bvt, N, \bx]$
\end{enumerate}
If one is only interested in the demographic rate parameters, $\bvt$, $N$, or derived quantities of these parameters, then one only needs steps 1 and 2. However if one is interested in other time-specific abundance quantities step 3 is necessary. 
...
The first step for whatever type of inference we want is to sample the $z_{it}$ for the observed individuals from the predictive distribution, $[\bz_{i}| \bx_i,\bvt]$. This can be accomplished efficiently with the Forward--Filtering Backward--Sampling (FFBS) algorithm for conditional state sampling of HMMs \citep{scott2002bayesian}. The details of the of the FFBS algorithm can be found in Appendix A (algorithm 1). Heuristically, the algorithm works by first using the HMM forward algorithm \citep{zucchini2016hidden} followed by a backward pass through the capture history to sample from $[z_{i,t}|z_{i,t+1},\dots,z_{i,K},\bx_i,\bvt]$ at each occasion. Once we perform the the backward-sampling algorithm for each captured individual we can calculate the derived quantity, $n_{t,l} = \sum_{i=1}^n I(z_{i,t}=s_l)$, the abundance of individuals in state $s_l$ at time $t$. For simplicity, we use the bold notation, $\bn_t = (n_{t,1},\dots,n_{t,J})$, to represent the vector of state-specific abundances and $n_t = \sum_{l=2}^{J-1} n_{t,l}$ to represent the total abundance of available individuals that were observed during the study.    
...
\end{array} \right.
\]
where $d_t \propto E[N_t/N_1|\bt, N_1]$ is the expected relative abundance on occasion $t$, and $f_t$ is the {\it per capita} recruitment parameter. The quantity $d_t$ is calculated with the recursion, $d_{t} = d_{t-1}(\phi_{t-1} + f_{t-1})$ where $d_1 \equiv 1$. The benefit of this form is that it provides a biological driver for defining entry. Similar to recruitment, $\rho_t$ the constant $f_t=f$ model provides are realistic simplification of the full time-specific model. 

In addition to the fecundity parameterization we also use a hierarchical approach for estimation of the  parameters. The hierarchical model is adds the prior and hyperprior components,
\begin{center}
$[\text{logit } \phi_t] = N(\mu_\phi,\sigma_\phi^2)$ \\
$[\text{logit } p_t] = N(\mu_p,\sigma_p^2)$, and $[\text{log } f_t] = N(\mu_f,\sigma_f^2)$
\end{center}
To allow shrinkage of the full time model to the temporally constant model we use the complexity prior distribution \citep{xxx} on the variance components, $[\sigma] =\text{Exponential}(1)$. All mean components of the random effect where given the vague $N(0,10)$ prior distribution. 

To fit this model with MCMC we used the {\tt R} package {\tt nimble} \citep{xxx} and developed several additional functions in the R package {\tt nimbleJSextras} for executing the fully marginal JS model. The {\tt nimbleJSextras} package is available for installation from GitHub\footnote{\url{https://github.com/dsjohnson/nimbleJSextras}}. The {\tt nimbleJSextras} package uses functionality in the {\tt nimble} \citep{xxx} and {\tt nimbleEcology} \citep{xxx} packages to fit general HMMs. Using the functions and distributions in those two packages we included the JS conditional HMM likelihood functions as well as functions for simulating true states for detected and undetected individuals. Code for this example is provided in the package and the dipper data was obtained from the {\tt marked} package \citep{xxx}. In addition, code to fit the fully marginal version of the specification in \cite{Royle:2008kx} is also provided for comparison. 

 We omitted the \cite{Royle:2008kx} analysis estimates here because they are virtually the same (outside of Monte Carlo error) as provided in their Table 10.xxx. This was the expected result as we fit the same model with the same prior distributions. Parameter estimates for the \cite{link2005modeling} parameterization are given in Table \ref{tab:dipper}. The super population size estimate is $N = 327$ with SD = 17 and 95\% HPD interval [300--359]. Yearly abundance estimates are illustrated in Figure \ref{}. 
