...
% \]
We prefer the recruitment probability parameterization of the JS model because the constant model, i.e., $\rho_t=\rho$, has biological interpretation about recruitment over time, however the constant model for $\beta_t$ implies changing recruitment probabilities that are not readily apparent. 
% An alternative $\beta_t$ model is presented by \cite{link2005modeling} based on {\it per capita} recruitment parameters. We will not consider that model here, but it can just as easily be incorporated in the collapsed sampler. 
...
Let $\bx$ represent observed data, $\bz$ is augmented data that is not directly observed, and $\bt$ are the parameters of the model. The standard Gibbs sampler for the posterior distribution,
\[
[\bt,\psi,\bw_{aug}, \bz^+|\bx^+] \propto [\bx^+|\bz^+,\bw^+,\bt][\bz^+|\bt][\bw^+|\psi][\bt][\psi],
\]
proceeds by alternate sampling between the sequential conditional distributions: $[\bt|\bz, \bx]$ and $[\bz|\bt,\bx]$. Throughout the paper, we use the ``$[A|B]$'' notation to represent a conditional density (distribution) function of $A$ given $B$.

\subsection{A heuristic view of collapsed Gibbs sampling for Jolly-Seber models}

Collapsed Gibbs samplers offer a method to improve MCMC convergence over standard Gibbs samplers. \cite{Liu1994a} provides a mathematical proof which shows that the rate of convergence for a collapsed Gibbs sampler is at least as good, but often better, than the standard Gibbs sampler for any particular model. Unfortunately, we cannot use the theoretical results to prove that the collapsed version will always converge at a faster rate because neither the augmented version or the collapsed version presented here use strictly Gibbs updates. That is at least some, if not all, of the parameters are updated using Metropolis-within-Gibbs updates because the full conditional distributions are not standard. However, \cite{VanDykJiao2015a} note that MCMC convergence for collapsed samplers using Metropolis updates can still offer improvement but it may not be as good as strictly Gibbs updates.  
In general a collapsed samplers is constructed by marginalizing over some components of the model, using a standard Gibbs sampler to sample the remaining parameters, then the marginalized parameters are sampled using their posterior predictive distribution given the first sample. 

For the time being, we will present a very heuristic description of the collapsed Gibbs sampler for augmented models. Details for JS models will be added in subsequent sections.  The collapsed Gibbs sampler begins by drawing from the marginal posterior distribution,
\[
...
\subsection{Calculating marginal likelihoods of Jolly-Seber models}
...
0 & [x_{it} | z_{it}=s_2] & 0 & 0 & 0 & s_2\\
0 &  0 & \ddots & 0 & 0 & \vdots \\
0 &  & 0 & [x_{it} | z_{it}=s_{J-1}] & 0 & s_{J-1}\\
...
\[
[x_{it}|z_{it}=s_l] = (1-p_{t,l})I(x_{it}=0) + p_{t,l}\text{Categorical}(x_{it}|\bd_l,x_{it}\ne0)
\]
