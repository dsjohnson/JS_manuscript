...
\]
where $\mathbf{0}$ is a $2m+3$ vector of zeros. The observation matrices, $\bP(x_{i,t})$, are still diagonal with with $[x_{i,t}|s_l]$ entries. However, because the detection process has been absorbed in the states, $[x_{i,t}|s_l]$, is just an indicator that the observation $x_{i,t}$ is possible in state $s_l$. For example $[x_{i,t}=0|s_l=R_{00}]=1$ and  $[x_{i,t}=0|s_l=R_{01}]=0$. 

Now that we the general form of the HMM matrices, we can define specific components for this analysis. Honu are long-lived with very stable survival once they reach adulthood \citep{PiacenzaEtAl2016a} so we modeled the exit probability as constant over time and between states, $\phi_{s_l,t} = \phi$ with prior distribution $[\text{logit }\phi] = t(\sigma=1.52,\text{df}=6)$, where $t(\sigma,\text{df})$ is a scaled $t$ distribution with df degrees of freedom. We used this specification to closely approximate a uniform distribution for $\phi$ but the sampled parameter, $\text{logit }\phi$, is unconstrained. 
%\[
...
\section{Code template for fitting a basic Jolly-Seber model using collapsed Gibbs sampling} 
