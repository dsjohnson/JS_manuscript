...
One of the major distinguishing features of JS family models relative to other capture-recapture models, such as Cormack-Jolly-Seber (CJS) models, is the entry probability. \citet{schwarz1996general} and \cite{crosbie1985parsimonious} parameterized entry in the JS model using marginal multinomial probabilities, $\xi_t$, $t=1,\dots,K$,  which are the unconditional probabilities that an individual enters between occasions $t-1$ and $t$ and is available for capture on occasion $t$. The first probability, $\xi_1$ is the probability that an individual is available at the first occasion. There are many ways to think about entry probabilities but we will base our discussion from this point. The main thing to consider when forming an conditional entry model is that all individuals must enter before occasion $K$, so the probabilities must reflect this constraint. Using the $\xi_t$ entry probabilities we can form the {\it conditional} entry probabilities that will used in the JS model computations herein,
\[
...
\[
\epsilon_t \propto \left\{ \begin{array}{ll}
1  & t=1\\
...
\subsection{Multistate abundance of nesting marine turtles}
